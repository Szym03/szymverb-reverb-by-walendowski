...
int main()
{
    ...
    while (1) // infinite loop until sentinel value entered
    {
        printf(">");
        scanf("%d", &x); // receiving input
        if (x == -1)     // sentinel value
        {
        ...
        if (x < 0 || x > 127)
        {
            ...
            continue; // start loop again, otherwise it prints the octaves for the wrong pitch anyway.
        }
        if (x > 0 || x < 127)
        {
