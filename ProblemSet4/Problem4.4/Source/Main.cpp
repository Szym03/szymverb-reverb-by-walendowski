...
{   
    ...
    for (int sample = 0; sample < inBuffer.getNumSamples(); ++sample)
    {
        for (int channel = 0; channel < inBuffer.getNumChannels(); ++channel)
        {
            ...
            channelWriteData[sample] = channelReadData[sample] * (0 + (sample - 0) * ((0.1 - 0) / (Numsamps/10.0 - 0))) * (1 + (sample - Numsamps/10.0) * ((0 - 1.0) / (Numsamps - Numsamps/10.0 )));
        }
    }
}
...
int readSoundFile (juce::AudioBuffer<float> *buffer)
{
    ...
    juce::File file("/Users/Szymon/Documents/GitHub/EP-353_ProblemSets/ProblemSet4/Problem4.4/Sine.wav");
...
int writeSoundFile (juce::AudioBuffer<float> buffer, float sampleRate)
{
    ...
    juce::File file("/Users/Szymon/Documents/GitHub/EP-353_ProblemSets/ProblemSet4/Problem4.4/enveloped.wav");
