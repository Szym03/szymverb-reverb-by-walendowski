...
void process (juce::AudioBuffer<float> inBuffer, juce::AudioBuffer<float> *outBuffer)
{   
    int numSamp = inBuffer.getNumSamples();
    for (int sample = 0; sample < inBuffer.getNumSamples(); ++sample)
    {
        for (int channel = 0; channel < inBuffer.getNumChannels(); ++channel)
        {
            ...
            channelWriteData[sample] = channelReadData[numSamp - 1 - sample];
        }
    }
}
...
int readSoundFile (juce::AudioBuffer<float> *buffer)
{
    ...
    juce::File file("/Users/Szymon/Documents/GitHub/EP-353_ProblemSets/ProblemSet4/Problem4.1/SN.wav");
